...
return new class extends Migration
{
    public function up(): void
    {
        Schema::table('service_applications', function (Blueprint $table) {
            if (! Schema::hasColumn('service_applications', 'sla_started_at')) {
                $table->timestamp('sla_started_at')->nullable()->after('priority');
            }

            if (! Schema::hasColumn('service_applications', 'appointment_at')) {
                $table->timestamp('appointment_at')->nullable()->after('sla_due_at');
            }

            if (! Schema::hasColumn('service_applications', 'appointment_location')) {
                $table->string('appointment_location')->nullable()->after('appointment_at');
            }

            if (! Schema::hasColumn('service_applications', 'appointment_message')) {
                $table->text('appointment_message')->nullable()->after('appointment_location');
            }

            if (! Schema::hasColumn('service_applications', 'appointment_status')) {
                $table->string('appointment_status')->nullable()->after('appointment_message');
            }
        });
    ...
    {
        Schema::dropIfExists('service_application_appointments');
